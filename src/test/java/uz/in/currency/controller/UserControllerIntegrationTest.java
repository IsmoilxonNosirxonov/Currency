...
public class UserControllerIntegrationTest {
    ...
    public void successfullyTestSignUp() throws Exception {
        ...
        String contentAsString = mockMvc.perform(MockMvcRequestBuilders.post("http://localhost:8083/api/auth/sign-up").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(createDto))).andExpect(MockMvcResultMatchers.status().isOk()).andDo(print()).andReturn().getResponse().getContentAsString();

        assertNotNull(contentAsString);
//        var response = objectMapper.readValue(contentAsString, new TypeReference<User>() {});
...
//        System.out.println("Result: "+objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(response));
    ...
    public void failedTestSignUp() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("http://localhost:8083/api/auth/sign-up").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(createDto))).andExpect(MockMvcResultMatchers.status().isInternalServerError()).andExpect(exceptionResult -> {
    ...
    public void successfullyTestSignIn() throws Exception {
        ...
        String contentAsString = mockMvc.perform(MockMvcRequestBuilders.post("http://localhost:8083/api/auth/sign-in").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(signInDto))).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.jsonPath("$.token").exists()).andDo(print()).andReturn().getResponse().getContentAsString();
    ...
    public void failedTestSignIn() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("http://localhost:8083/api/auth/sign-in").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(signInDto))).andExpect(MockMvcResultMatchers.status().isInternalServerError()).andExpect(exceptionResult -> {
            assertNotNull(exceptionResult);
